...
export function createApiRouter(
    ...
    router.post("/submission/:agentId", async (req, res) => {
        const agentId = req.params.agentId;
        const agentRuntime = agents.get(agentId);
        if (!agentRuntime) {
            res.status(404).json({ error: "Agent not found" });
        ...
        const message: Memory = {
            roomId: stringToUuid("PROMOTE_TOKEN_ACTION_ROOM"),
            userId: agentRuntime.agentId,
            agentId: agentRuntime.agentId,
            content: {
                action: "VERIFY_ACTION",
                text: `NFT ID: ${req.body.nftId} Content: ${req.body.content}`,
            },
        };
        ...
        let success;
        await agentRuntime.processActions(
            message,
            [message],
            state,
            async (response) => {
