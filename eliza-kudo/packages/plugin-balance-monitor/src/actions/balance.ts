...
export const balanceAction: Action = {
    ...
    description:
        "Creates a goal for an agent to maintain a balance above a fixed amount of tokens on a chain for trading activity",
    ...
    handler: async (
        ...
        const goal: Goal = {
            ...
            objectives: [
                {
                    description: `Mint CovenantNFT if agent balance is below ${response.amount} ${response.token}`,
                    completed: false,
                },
            ],
        };
