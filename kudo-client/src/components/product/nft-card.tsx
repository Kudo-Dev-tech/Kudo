...
export function NftCard({
  ...
  const chainId = getChainIdFromCAIP2(wallets[0].chainId);
  const cnftAddress = getContractAddress(chainId, "covenantNFT");
  ...
  const displayData = {
    ...
    nftMarketplaceLink: isCovenantLoading
      ? covenantDetails.nftMarketplaceLink
      : covenantInfo?.nftMarketplaceLink || covenantDetails.nftMarketplaceLink,
  };
    ...
    >
      ...
      <CardFooter className="flex flex-col gap-3">
        ...
        {displayData.status !== Status.Minted
          ? displayData.nftMarketplaceLink && (
              <Link
                href={displayData.nftMarketplaceLink}
                target="_blank"
                className="w-full"
              >
                <Button
          ...
          : displayData.nftType === Category.Loan && (
              ...
              >
                Buy for{" "}
                {formatUnits(displayData.price, covenantDetails.asset.decimals)}{" "}
                {covenantDetails.asset.symbol}
              </Button>
            )}
      </CardFooter>
    </Card>
